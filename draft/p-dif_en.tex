...
The outline of the computational procedures is given as follows along
...
The buoyancy-production term in the prognostic equation of the twice
...
where \(R\) and \(Q_l\) are cloud amount and liquid water computed from
...
Based on Holtslag and Boville (1993), \(H_{PBL}\) is estimated using the
...
The twice turbulent kinetic energy of the level 2, \({q_2}^2\), is
...
(\href{p-dif.5}{5}), \(q_2^2\) is calculated by
...
Using (\href{p-dif.14}{14}), (\href{p-dif.17}{17}) and the definition of
...
diagnostically from (\href{p-dif.6}{6})-(\href{p-dif.8}{8}),
...
(\href{p-dif.20}{20}) and (\href{p-dif.21}{21}) lead to
...
(\href{p-dif.22}{22}) makes the following matrix equation,
...
(\href{p-dif.23}{23}) is solved for \(\mu_k\) using the LU
decomposition.

\hypertarget{tendencies-of-other-prognostic-variables}{%
\paragraph{Tendencies of other prognostic
variables}\label{tendencies-of-other-prognostic-variables}}
...
These equations lead to (\href{p-dif.23}{23}) again and computed with
