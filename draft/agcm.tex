\documentclass[11pt,a4paper,onecolumn]{jarticle}
%
\usepackage[top=30truemm,bottom=30truemm,left=25truemm,right=25truemm]{geometry}
\usepackage{amsmath,amssymb}
\usepackage{bm}
\usepackage[dvipdfmx]{graphicx}
\usepackage{ascmac}
\usepackage{longtable}
\usepackage{booktabs}
\usepackage{ltablex,booktabs}
\usepackage[dvipdfm]{hyperref}
%

\title{\Huge Description for MIROC6}
\author{}
\date{\today}
\pagenumbering{arabic}
%
%
%
\begin{document}
%
	\maketitle
	\tableofcontents
	\clearpage
	%
	\def\tightlist{\itemsep1pt\parskip0pt\parsep0pt}
	% please change the line below as your tex file (without ".tex")
	% agcm の概念と構造
	% \include{a-intro} % 渡部先生が書く
	% 基本設定
	\include{a-setup}
	% Computational flow of dynamical core
	\include{d-routine}
	% 基本方程式
	\include{d-basic}
	% 力学:鉛直離散化
	\include{d-vert}
	% 力学:水平離散化
	\include{d-hori}
	% 力学:時間離散化
	\include{d-time}
	% トレーサー
	\include{dynamics_jan.tex}
	% カプラー
	\include{AO-couple.tex}
	\include{Model-Grid.tex}
	% 力学:拡散項等
	% \include{d-diff} % 元々存在しない？
	% 力学:まとめ
	\include{d-summ}
	% 物理過程:イントロ
	%\include{p-intro} % 渡部先生が書く
	% 物理過程:cumulus scheme
	\include{p-cum}
	% 物理過程:shallow convection scheme
	\include{shallowconv_en}
	% 物理過程:大規模凝結
	\include{Hotta_pmlsc}
	% 物理過程:雲微物理
	\include{Hotta_pcldphys}
	% 物理過程:放射
	\include{p-rad}
	% 物理過程:拡散フラックス
	\include{p-dif_en}
	% 物理過程:地表フラックス
	% \include{p-sflx} % 廃止？
	% 物理過程:地表モデル
	\include{p-sfc}
	% 物理過程:implicit 解法
	% \include{p-solv} % 担当者なし
	% 物理過程:重力波抵抗
	% \include{p-grav} % 担当者なし
	% 物理過程:対流調節
	% \include{p-adj} % 廃止
	% 参考文献
	% \include{referenc} % まだ作っていない
	\include{d-ref}
	%
	%
\end{document}

