...
cloud water mixing ratio. \(q_t\) is sum of water vapor and cloud water
mixing ratio. \(\delta(x)\) denotes the Heviside function of x.
...
The scheme predicts variance (\(V\)) and skewness (\(S\)) of the PDF.
...
convection, cloud microphysics and turbulent mixing processes
respectively, which all affect the PDF shape. The last terms represent
dissipation due to subgrid-scale horizontal motions. The specific
formulations for each term are described below.
...
evaporation,and melting/freezing.
