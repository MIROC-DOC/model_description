...
##  **Total optical thickness [in DTRN31]**
...
\begin{equation}
\begin{array}{l}
\hat{\epsilon}_{S}^{-}=F_{\text {sol }}\left(V_{s}^{-}-R V_{s}^{+}-T V_{s}^{-} e^{-\frac{\tau^{*}}{\mu_{0}}}\right) \\
...
\end{equation}
$$
$Ex$ is the cumulative direct solar transmission. $B_{n}^{(1-4)}$ is the cloud cover interaction index and calculated in subroutine BCVR.
$$
\begin{equation}
\begin{array}{l}
...
B_{k}^{(4)}=\frac{\min \left(C_{n+1}, C_{n}\right)}{C_{n+1}}
\end{array}
\end{equation}
$$
...
\begin{equation}
\begin{array}{c}
F_{n+1 / 2}^{+}=\frac{W^{+}}{W}\left(u_{n+1 / 2}^{+(c l o u d)}+u_{n+1 / 2}^{+(c l e a r)}\right)+\mu_{0} F_{0}\left(e_{n+1 / 2}^{-\left\langle\tau^{*}\right\rangle / \mu_{0}(\text { cloud })}\right) \\
F_{n+1 / 2}^{-}=\frac{W^{+}}{W}\left(u_{n+1 / 2}^{-(c l o u d)}+u_{n+1 / 2}^{-(\text {clear })}\right)
\end{array}
\end{equation}
$$
...
## **Flux of incidence and incident angle [SHTINS]**
...
##  **Reading the each parameter [OPPARM2]**

In this subroutine, various parameters used for radiation calculation are read. The outline of the procedure is shown below.
